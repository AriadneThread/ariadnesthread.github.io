...
6. 两个操作数都是对象，则比较他们是否是同一个对象，
   如果都指向同一个对象，则返回 true，否则返回 false。
...
**虽然，它比较基于底层，性能和速度会非常好，而就是因为比较底层，使用的难度也很大。** 所以，我们我们如果不做框架级的开发，不用理会它。位运算符有七种，分别是：
**位非 NOT(~)、位与 AND(&)、位或 OR(|)、位异或 XOR(^)、左移(<<)、有符号右移(>>)、无符号右移(>>>)。**
